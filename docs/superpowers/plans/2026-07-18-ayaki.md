...
- Task 12: Inspector morpheme {#each} made unkeyed — the plan's key (m.surface + m.posJa) throws Svelte's each_key_duplicate on repeated identical morphemes (e.g. ！！).
- Task 15: kuromoji/zlibjs are incompatible with Vite's ESM bundling (blank page; see task-15 report). Fixed via browser-only Vite aliases to in-repo shims: pako-backed Gunzip with gzip-magic sniff (also neutralizes dev-server double-decompression) and a minimal path.join shim; pako added as runtime dep. Favicon added to index.html (404 noise).
- Final review: CI gained svelte-check + a Playwright browser smoke test (the unit suite is structurally blind to browser-bundling breakage); a11y fixes (svg role=group, unique marker ids, per-morpheme speak labels, aria-pressed); misc polish (license field, engines >=20.19, path shim dirname, retry-path test).



- Post-plan (user request): NodeTree now renders furigana when the toggle is on — the spec had scoped furigana to the arc diagram only, which was an unconsidered gap rather than a decision.
